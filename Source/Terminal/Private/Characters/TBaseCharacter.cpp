...
ATBaseCharacter::ATBaseCharacter()
{
	...
	FileDataComponent = CreateDefaultSubobject<UTFileData>(TEXT("File Data Component"));

	OnCharacterMovementUpdated.AddDynamic(this, &ATBaseCharacter::Animate);
...
void ATBaseCharacter::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
...
void ATBaseCharacter::BeginPlay()
{
	...
	OnFootstepTakenNative.AddUObject(this, &ATBaseCharacter::OnFootstepNative);
}

void ATBaseCharacter::OnFootstepNative()
{
	if (bFootstepGate)
	{
...
void ATBaseCharacter::Animate(float DeltaTime, FVector OldLocation, const FVector OldVelocity)
{
...
void ATBaseCharacter::SetCurrentAnimationDirection(const FVector& Velocity, TOptional<FMinimalViewInfo> ViewInfo)
{
	FVector Forward;
	FVector Right;
	if (ViewInfo.IsSet())
